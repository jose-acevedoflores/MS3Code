/* $Id: sd_card.c,v 1.106.2.1 2013/03/24 15:37:50 jsmcortina Exp $
...
// FIXME need to add voltage protection here
    ...
    sd_match = SD_OK;
    sd_match_mask = 0xff;
    sd_int_addr = 0x1000;       // where to read from
    sd_int_cnt = 0;
    sd_int_error = 0;
    SS_ENABLE;
    (void) SPI1SR;
...
void do_sdcard(void)
{
    ...
    outpc.sd_phase = sd_phase;  // only temporary
    ...
    if (sd_phase == 0x41) {     // time to log?
        ...
        ul_tmp = lmms;
        ENABLE_INTERRUPTS;
        if ((ram4.log_style & 0x03) == 0x01) {  // stream mode
            if (flagbyte6 & FLAGBYTE6_SD_GO) {
                flagbyte6 &= ~FLAGBYTE6_SD_GO;
                if (((ram4.log_style & 0x03) == 0x01)
                    && (sd_subblock >= 2)) {
        ...
        if (ram4.log_style & 0x80) {    // button or WTF
            ...
            if (flagbyte6 & FLAGBYTE6_SD_DEBOUNCE) {    // is button released yet
                ...
                if ((*port_sdbut & pin_sdbut) == pin_match_sdbut) {
                    ...
                    ul_tmp = lmms;
                    ENABLE_INTERRUPTS;
                    if ((lmms - sd_lmms_last2) > 7812) {        // ignore button until 1s after last action
...
// have to honour size even if wrong somehow
        ...
        *(unsigned char *) (st_ad + 0x3f) = sd_magic;   // write magic number into every datablock
        ...
        if (((ram4.log_style & 0x03) == 0x01) && (sd_subblock >= 2)) {
            sd_phase = 0x47;    // dummy phase to prevent any more blocks over-writing stream data
        ...
        if (sd_int_phase == 0) {        // write block phase
            ...
            if (ATD0DR4 > MIN_VOLTS) {  // > 6.5V to avoid writing during power fail which might corrupt card
                ...
                if (sd_log_addr == 0x1000) {
                    sd_log_addr = 0x1200;       // this is also hardcoded and reflects 512byte sector - which should be safe
                } else {
                    sd_log_addr = 0x1000;
                }
...
// dummy phase 0x47
        ...
        if (PTH & 0x20) {       // card no longer present
            ...
            sd_log_addr = 0x1000;       // start with first block
...
// returns back here to step 0x08
        ...
        sd_match = SD_OK;
        sd_match_mask = 0xff;
        sd_int_addr = 0x1000;   // where to store the results.
        sd_int_cnt = 0;
        sd_int_error = 0;
        SS_ENABLE;
        (void) SPI1SR;
        ...
        // check for 55AA
        if (*((unsigned int *) 0x11fe) != 0x55AA) {     //byte 0x1fe
            outpc.sd_error = 7;
            sd_phase = 0xfe;
            goto SD_end_here;
        }
        ...
        *(unsigned char *) &sd_part_start = *((unsigned char *) 0x11c9);
        *((unsigned char *) &sd_part_start + 1) = *((unsigned char *) 0x11c8);
        *((unsigned char *) &sd_part_start + 2) = *((unsigned char *) 0x11c7);
        *((unsigned char *) &sd_part_start + 3) = *((unsigned char *) 0x11c6);


        if (sd_part_start == 0x00000000) {
            /* weak detection of VBR vs. MBR */
            if (((*((unsigned char *) 0x1026) == 0x29)
                && (*((unsigned char *) 0x1036) == 'F')
                && (*((unsigned char *) 0x1037) == 'A')
                && (*((unsigned char *) 0x1038) == 'T')
                && (*((unsigned char *) 0x1039) == '1')
                && (*((unsigned char *) 0x103a) == '6'))

                || (((*((unsigned char *) 0x1042) == 0x29))
                && (*((unsigned char *) 0x1052) == 'F')
                && (*((unsigned char *) 0x1053) == 'A')
                && (*((unsigned char *) 0x1054) == 'T')
                && (*((unsigned char *) 0x1055) == '3')
                && (*((unsigned char *) 0x1056) == '2'))) { // found FAT16/32 sigs, assume VBR
                    sd_part_start = 0;
        ...
        part_type = *((unsigned char *) 0x11c2);
        ...
        if (!((part_type == 0x0b) || (part_type == 0x0c)
            ...
            if ((*((unsigned char *) 0x11be) & 0x7f) == 0) {
        ...
        if ((*((unsigned char *) 0x11be) & 0x7f) || (*((unsigned char *) 0x11ce) & 0x7f)
            || (*((unsigned char *) 0x11de) & 0x7f) || (*((unsigned char *) 0x11ee) & 0x7f)) {
            // partition table not valid
            // How are we supposed to be sure this is a VBR???
            if ((*((unsigned char *) 0x11d2) == 'e') && (*((unsigned char *) 0x11d3) == 'r')
                && (*((unsigned char *) 0x11d4) == 'r') && (*((unsigned char *) 0x11d5) == 'o')
                && (*((unsigned char *) 0x11d6) == 'r') && (*((unsigned char *) 0x11d7) == 0xff)) {
                ...
                if ((*((unsigned char *) 0x1026) == 0x29) || ((*((unsigned char *) 0x1042) == 0x29))) { // expected FAT16/32 sigs
                    sd_part_start = 0;
                    outpc.sd_status |= 0x80; // VBR
                    sd_phase++; // go forwards without reading new data.
                    goto SD_end_here;
        ...
        sd_sect2clust = *((unsigned char *) 0x100d);

        sd_fat_start =
            sd_part_start + (*((unsigned char *) 0x100e) +
                             (*((unsigned char *) 0x100f) << 8));
        // offset 0x0e is little-endian word num of sectors reserved for boot sector before FAT starts

        // Max roots - only for FAT16
        *(unsigned char *) &sd_max_roots = *((unsigned char *) 0x1012);
        *((unsigned char *) &sd_max_roots + 1) = *((unsigned char *) 0x1011);

        // sectors per FAT
        *((unsigned char *) &sd_fatsize + 3) = *((unsigned char *) 0x1016);
        *((unsigned char *) &sd_fatsize + 2) = *((unsigned char *) 0x1017);
        *((unsigned char *) &sd_fatsize + 1) = 0;       // clear top word
        *((unsigned char *) &sd_fatsize + 0) = 0;       // clear top word

        sd_nofats = *((unsigned char *) 0x1010);

        if ((sd_max_roots == 0) || (sd_fatsize == 0)) {
            if (*((unsigned char *) 0x1042) == 0x29) {
                unsigned long ult;
                outpc.sd_status |= 0x40;        // FAT32 detected
                *(unsigned char *) &sd_fatsize = *((unsigned char *) 0x1027);
                *((unsigned char *) &sd_fatsize + 1) = *((unsigned char *) 0x1026);
                *((unsigned char *) &sd_fatsize + 2) = *((unsigned char *) 0x1025);
                *((unsigned char *) &sd_fatsize + 3) = *((unsigned char *) 0x1024);

                // cluster start of root directory
                *(unsigned char *) &ult = *((unsigned char *) 0x102f);
                *((unsigned char *) &ult + 1) = *((unsigned char *) 0x102e);
                *((unsigned char *) &ult + 2) = *((unsigned char *) 0x102d);
                *((unsigned char *) &ult + 3) = *((unsigned char *) 0x102c);
        ...
        if (*((unsigned int *) 0x1013) == 0) {
            *(unsigned char *) &sd_tot_sect = *((unsigned char *) 0x1023);
            *((unsigned char *) &sd_tot_sect + 1) = *((unsigned char *) 0x1022);
            *((unsigned char *) &sd_tot_sect + 2) = *((unsigned char *) 0x1021);
            *((unsigned char *) &sd_tot_sect + 3) = *((unsigned char *) 0x1020);
        } else {
            *(unsigned char *) &sd_tot_sect = *((unsigned char *) 0x1014);
            *((unsigned char *) &sd_tot_sect + 1) = *((unsigned char *) 0x1013);
        }
        ...
        // first sector of file now at 0x1000
            if ((*((unsigned long *) 0x1000) == 0x00000000)
                && (*((unsigned long *) 0x1004) == 0x00000000)) {
                // we got a pre-made empty file - hurrah!
                sd_maxlogblocks = sd_filesize_bytes / LOG_SIZE;
                if (ram4.log_style & 0x80) {
                    sd_phase = 0x2b;    /* go wait for button */
                } else {
...
//                    outpc.sd_status |= 0x04;        // ready
        ...
        if ((ram4.log_style & 0x03) == 0x01) {  // 64 byte + stream
            sd_maxlogblocks = sd_maxlogblocks / 192;    // interval is ~192 timer ticks in stream mode
        } else {
            sd_maxlogblocks = sd_maxlogblocks / ram4.log_int;   // max number of sample blocks
        }
        ...
        while (!j) {
            ...
            if (outpc.sd_status & 0x40) {
                y = *((unsigned long *)
                      &(*(unsigned char *) (0x1000 + x)));
            } else {
                y = *((unsigned int *) &(*(unsigned char *) (0x1000 + x)));
            }
            if (y == 0) {
        ...
        unsigned char j = 0;
        // now we have the starting sector of FAT we want to write loaded at 0x1000
        ...
        if (outpc.sd_status & 0x40) {
            ...
            while (!j) {
                ...
                *(unsigned char *) (0x1000 + sd_subblock) = ct;
                ct = (sd_ultmp & 0xff00) >> 8;  // high byte
                *(unsigned char *) (0x1001 + sd_subblock) = ct;
                ct = (sd_ultmp & 0xff0000) >> 16;       // high byte
                *(unsigned char *) (0x1002 + sd_subblock) = ct;
                ct = (sd_ultmp & 0xff000000) >> 16;     // high byte
                *(unsigned char *) (0x1003 + sd_subblock) = ct;
                ...
                if (sd_clust_cnt >= sd_file_numclust) {
                    ...
                    *(unsigned long *) &(*(unsigned char *) (0x0ffc + sd_subblock)) = 0xffffff0f;       // end of file marker
                    j = 1;
            ...
            while (!j) {
                ...
                *(unsigned char *) (0x1001 + sd_subblock) = ct;
                ...
                if (sd_clust_cnt >= sd_file_numclust) {
                    ...
                    *(unsigned int *) &(*(unsigned char *) (0x0ffe + sd_subblock)) = 0xffff;    // end of file marker
                    j = 1;
                } else if (sd_subblock >= sd_sectsize) {
                    j = 2;
                }
...
// FIXME need to add voltage protection here
        ...
        unsigned int ad;
        for (ad = 0x1000; ad < 0x1200; ad++) {  // clear buffer
            *((unsigned int *) ad) = 0;
        }
...
//        if ((ram4.log_style & 0xc0) == 0xc0) { // WTF mode
//            outpc.sd_status |= 0x84;
//            sd_phase = 0x40; // go through process of recording, but don't write
//        } else { // button only
...
//        }
        ...
        // very short press is WTF, longer press is normal
...
/* end debug */
    ...
    /* --------- */
        ...
        if (((ram4.log_style & 0x03) != 0)
            && ((ram4.log_style & 0x03) != 1)) {
            sd_phase = 0;
        ...
        *(unsigned char *) (sd_log_addr + 0x18) = ram4.log_style & 0x03;        // log block size code
        *(unsigned char *) (sd_log_addr + 0x19) = 1;    // format revision
        if ((datax1.rtc_year > 2010) && (datax1.rtc_sec < 60)) {
            // looks like a real time/data
        ...
        *(unsigned char *) (sd_log_addr + 0x7f) = sd_magic;
        ...
        if (ram4.log_int < 24) {        // minimum is 3ms due to time to write block
            sd_lmms_int = 24;
        } else {
        ...
        sd_lmms_last = lmms;
        ENABLE_INTERRUPTS;
        sd_stream_ad = 0x1080;  // more space for stream
        sd_stream_cnt = 1;
        sd_phase++;
        ...
        *(unsigned int *) 0x1200 = sd_uitmp;    // sequence number
        sd_uitmp++;
        sd_pos++;
        ad = 0x1000;
        ...
        *(unsigned long *) 0x1200 = sd_pos;     // sequence number
        sd_pos++;
        sd_ledstat = 1;
        sd_phase = 0x2c;        // wait again
...
//        if ((sd_int_error == 8) && (sd_rx == 5)) {
        ...
        if (SPI1SR & 0x80) {
            *((unsigned char *) (0x1000 + sd_int_cnt)) = SPI1DRL;
            sd_int_cnt++;
            SPI1DRL = 0xff;     // dummy write
            if (sd_int_cnt == 4) {
                SS_DISABLE;
        ...
        // check the VCA
        if (*((unsigned char *) (0x1003)) != 0xaa) {    // check pattern doesn't match
            // retry command
            sd_nofats++;
            if (sd_nofats > 10) {
                outpc.sd_error = 21;
        ...
        } else if (*((unsigned char *) (0x1002)) != 1) {        // voltage rejected
            outpc.sd_error = 22;
            sd_phase = 0xfe;
            goto SD_end_here;
        } else {
...
// Changed for 0.26 to handle with a pause and a loop counter to prevent endless looping
        ...
        *((unsigned char *) (0x1000 + sd_int_cnt)) = SPI1DRL;
        ...
        if ((*((unsigned int *) (0x1000)) == 0xffff)
            && (*((unsigned int *) (0x1002)) == 0xffff)) {
            // didn't work, BS data
            SS_DISABLE;
            sd_nofats++;
            if (sd_nofats > 10) {
        ...
        if ((*((unsigned char *) (0x1000)) & 0x80) == 0) {
...
// ******** delete/erase file
        ...
        while (!j) {
            ...
            if (*(unsigned char*)ad == 0) {
                ...
                ((*(unsigned char*)(ad + 0x03)) == *(unsigned char*)0x1200) &&
                ((*(unsigned char*)(ad + 0x04)) == *(unsigned char*)0x1201) &&
                ((*(unsigned char*)(ad + 0x05)) == *(unsigned char*)0x1202) &&
                ((*(unsigned char*)(ad + 0x06)) == *(unsigned char*)0x1203) &&
...
//            tmp_fat_pos = (sd_ultmp * 2) % 512;
        ...
        if (sd_int_error) {
            ...
            if (sd_ultmp == 0xffffffff) {
                ...
                sd_pos++;
    //            sd_phase = 0x80; // do a dummy read and continue
    //            sd_match = 0x8c; // grab next FAT sector
                sd_phase = 0x8c;
            }
        }
...
// ******** speed test
        ...
        if (sd_pos == 0) {
            *(unsigned char*)0x100c = 2; // say there was an error
            // bail for now unless sector number supplied
            sd_phase = 0x2c;
        } else if (*(unsigned long*)0x1000 == 0) {
            // requested to test zero sectors ?!
            *(unsigned char*)0x100c = 2; // say there was an error
            sd_phase = 0x2c;
        } else {
            *(unsigned char*)0x100c = 0; // status = running
...
// space here for phases to allocate some test sectors
        ...
        unsigned int ix;
        // fill data buffer
        for (ix = 0x1200 ; ix < 0x1400 ; ix++) {
            *(unsigned char*)ix = (unsigned char)ix;
        }
        *(unsigned long*)0x1004 = 0; // total
        *(unsigned int*)0x1008 = 0xffff; // min
        *(unsigned int*)0x100a = 0; // max
        sd_phase++;
        ...
        sd_match = SD_OK;
        sd_match_mask = 0xff;
        sd_int_addr = 0x1200;   // where to read from
        sd_int_cnt = 0;
        sd_int_error = 0;
        SS_ENABLE;
        (void) SPI1SR;
        ...
        if (sd_int_error) {
            *(unsigned char*)0x100c = 0x01; // completed
            sd_phase = 0x2c;
        } else {        
        unsigned int tmp_time;
            tmp_time = (unsigned int)lmms - (unsigned int)sd_lmms_last;

            *(unsigned long*)0x1004 += tmp_time;
            
            if (tmp_time < *(unsigned int*)0x1008) {
                *(unsigned int*)0x1008 = tmp_time;
            }

            if (tmp_time > *(unsigned int*)0x100a) {
                *(unsigned int*)0x100a = tmp_time;
            }


            *(unsigned long*)0x1000 = *(unsigned long*)0x1000 - 1;

            if (*(unsigned long*)0x1000) {
                sd_pos++;
...
void SPI_baudhigh(void)
{
    ...
    if (sp == 0xc0) {
        SPI1BR = 0x34; // 390kHz (195) - only works on MS3 card with updated resistor values
    } else if (sp == 0x80) {
        SPI1BR = 0x44; // 312kHz (156) - "faster" works on JSM MS3 card, original standard speed
    } else if (sp == 0x40) {
        SPI1BR = 0x64; // 222kHz (111) - "slower"
    } else {
        SPI1BR = 0x54; // 260kHz (130) - the new "normal"
    }
