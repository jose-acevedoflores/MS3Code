/* $Id: ms3_ign_in.c,v 1.227.2.4 2013/05/22 13:48:01 jsmcortina Exp $
...
//Here we use RPAGE for the tooth/trigger log FIXME HARDCODED due to gcc bug
    ...
    } else if (flagbyte0 & FLAGBYTE0_MAPLOG) {
...

    {
        ...
        if (ram_window.pg24.vvt_opt1 & 0x03) { /* VVT enabled at all */
            ...
            if ((ram_window.pg24.vvt_opt4 & 0x03) == 1) { /* BMW V10 decoder */
...
//        goto SPKMODE51;
  ...
  SPKMODE4:
    ...
    if (!(synch & SYNC_SYNCED)) {
            ...
            if (tooth_diff_this >= temp1) {
                ...
                if ((ram4.spk_config & 0x0c) == 0x0c) { // dual + missing
                    ...
                    if ((ram4.spk_config & 0x30) == 0x30) { // poll level
                        ...
                        if ((mapadc_thresh) && (!(flagbyte2 & flagbyte2_crank_ok))) {
                            flagbyte1 &= ~flagbyte1_trig2active; // Don't check phase until we've been running for a bit
                        }
                        // rising OR falling cam
                        if (!(flagbyte1 & flagbyte1_trig2active)) { // if we haven't seen the cam yet then do WCOP, else fully synced
                            synch |= SYNC_SEMI2;
        ...
        unsigned int mltp1 = mid_last_tooth + 1 + ram4.No_Miss_Teeth;
        ...
        if ((tooth_no == last_tooth) || ((tooth_no == mid_last_tooth) && ((ram4.spk_config & 0xc) == 0xc))) {
            ...
            if (tooth_diff_this <= temp1) {
                ...
                if (synch & SYNC_SEMI2) {
                    ...
                    // normal non WCOP mode
                    // SEMI2 and WCOP cases shouldn't happen with "poll level"
                    // if dual + missing & cranking (or just out crank) then double check cam is in phase
                    if (((outpc.engine & ENGINE_CRANK) || (flagbyte2 & flagbyte2_crank_ok)) && ((ram4.spk_config & 0xc) == 0xc)) {
                        if ((ram4.spk_config & 0x30) != 0x30) { /* non poll level */
                            if ((tooth_no == last_tooth) && (!(flagbyte1 & flagbyte1_trig2active))) {
                                // was expecting to see the cam, but it wasn't here..
                                return(0);
                            } else if ((tooth_no == mid_last_tooth) && (flagbyte1 & flagbyte1_trig2active)) {
                                // wasn't expecting to see the cam and but did, reset ignition and restart
                                outpc.syncreason = 17;
                                ign_reset(); // will fire any pending coils
                                synch |= SYNC_SYNCED;
                                outpc.status1 |= STATUS1_SYNCFULL;
                                tooth_no = 0;
                            }
        ...
        } else if ((tooth_no == 1) || (tooth_no == mltp1)) {
            /* re-check poll level and tooth no.2 to be consistent */
            if (!(synch & SYNC_SEMI2) && !(synch & SYNC_WCOP)) {
                // normal non WCOP mode
                // SEMI2 and WCOP cases shouldn't happen with "poll level"
                if (((outpc.engine & ENGINE_CRANK) || (flagbyte2 & flagbyte2_crank_ok))
                    && ((ram4.spk_config & 0xc) == 0xc)
                    && ((ram4.spk_config & 0x30) == 0x30)) { /* poll level re-check */
                    if (ram4.spk_config & 0x01) {      // high cam for phase 1
                        if ((PORTT & TFLG_trig2) == 0) {        // low so phase 2
                            if (tooth_no != mltp1) { /* must be last_tooth */
                                outpc.syncreason = 18;
                                ign_reset(); // will fire any pending coils
                                tooth_no = mid_last_tooth;
                                outpc.status1 |= STATUS1_SYNCFULL;
                                synch |= SYNC_SYNCED;
                            }
                        } else {
                            if (tooth_no != 1) { /* must be mid_last_tooth */
                                outpc.syncreason = 18;
                                ign_reset(); // will fire any pending coils
                                tooth_no = 0;
                                outpc.status1 |= STATUS1_SYNCFULL;
...
*/
  ...
  SPKMODE3:
    ...
    if ((spkmode == 2) && ((ram4.spk_conf2 & 0x06) == 0x06)) {
        // sig-PIP TFI
        if (!edge) {
  ...
  SPKMODE7:
    ...
    if (!(synch & SYNC_SYNCED)) {
        ...
        if (!(synch & SYNC_SEMI)) {     // just starting
            ...
            } else if (((tooth_no > 30) && (flagbyte5 & FLAGBYTE5_CAM))
                       || (tooth_no > 60)) {
                // PS Deviant track 9
                outpc.syncreason = 29;
                ign_reset();
...
SM11_SYNC:;
  ...
  SPKMODE13:
    ...
    if (!(synch & SYNC_SYNCED)) {
        ...
        if (tooth_no == 3) {
            ...
            if ((!(edge)) || (utmp13 == (TFLG_trig2 | 0x01))
                || (utmp13 == 0x00)) {
                outpc.syncreason = 39;
                ign_reset();
                return(0);
  ...
  SPKMODE14:
    ...
    /************ Chrysler 2.2/2.5 *************/
  SPKMODE15:
    /* Rewritten May 2012. Now uses just the crank signal and a single edge.
      Stricter syncing and re-syncing. */
    if (!(synch & SYNC_SYNCED)) {

        if (tooth_diff_this && tooth_diff_last && tooth_diff_last_1) {
            /* Find the two short teeth following a long tooth 180-130-50 */
            /* Beware of confusing with normal engine acceleration during cranking */
            if ((tooth_diff_this < tooth_diff_last) && (tooth_diff_last < tooth_diff_last_1)
                && (tooth_diff_this < (tooth_diff_last_1 >> 1)) && (tooth_diff_last > (tooth_diff_last_1 >> 1))) {
                tooth_no = 1;
                outpc.status1 |= STATUS1_SYNCFULL;
                synch |= SYNC_SYNCED;
                outpc.status4 = 1; // which sensor gave sync DEBUG
            } else {
                return(0);
            }
        } else {
        ...
        // recheck for sync
        if (tooth_no == 1) {
            if (!((tooth_diff_this < (tooth_diff_last_1 >> 1)) && (tooth_diff_last < tooth_diff_last_1))) {
                outpc.syncreason = 42;
                ign_reset();
                return(0);
            }
...
//        if ((tooth_diff_last > ltmp1) && (tooth_diff_last_1 > ltmp1)) {
        ...
        if ((tooth_diff_this > ltmp1) && (tooth_diff_last_1 > ltmp1)) {
            if (cycle_deg == 7200) {   // COP or use-cam
                /* ls1_sl is used to store cam tooth pattern -> long-short =1 or short-long =2 */
                if (ls1_sl == 2) {
                    tooth_no = 7;
                    outpc.status1 |= STATUS1_SYNCFULL;
                } else if (ls1_sl == 1) {
                    ...
                    /* not presently a method to half-sync or run without cam*/
                    outpc.syncreason = 51;
                    ign_reset();
                    return(0);
                }
        ...
        if ((tooth_no == 6) || (tooth_no == 13)) {    // (one less)
            ...
            if ((cycle_deg == 7200) && ((outpc.engine & ENGINE_CRANK) || (flagbyte2 & flagbyte2_crank_ok))) {
                /* recheck cam phase at start only */
                if (((tooth_no == 6) && (ls1_sl != 2)) || ((tooth_no == 13) && (ls1_sl != 1))) {
                    outpc.syncreason = 81;
                    ign_reset();
                    return(0);
                }
...
SM43SS2:;
    ...
    goto common_wheel;
        ...
        /* Present implementation REQUIRES a cam signal.
           Assume cam leading edge precedes no.1
...
//                }
  ...
  SPKMODE52:
    ...
    if (!(synch & SYNC_SYNCED)) {
        ...
        /* resync code */
        if ((tooth_no == 10) || (tooth_no == 6) || (tooth_no == 20)) {
            if (!(flagbyte1 & flagbyte1_trig2active)) {
                ...
                outpc.syncreason = 93;
SPK52_LOST:
                outpc.status4 = tooth_no; // for debug
                ign_reset();
            } else if (tooth_no == 20) {
                tooth_no = 0;
            }
...
//                tooth_no = 1;
    ...
    if ((synch & SYNC_SYNCED) || (synch & SYNC_SEMI)) { 
        if ((spkmode == 5) || (spkmode == 6) || (spkmode == 40) || ((spkmode == 41) && (tooth_no == 1)) || ((spkmode == 2) && ((ram4.spk_conf2 & 0x06) == 0x06)))  {
    ...
    if ((spkmode & 0xfe) == 2) {  // 2,3
        ...
        if ((ram4.ICIgnOption & 0x08) || ((spkmode == 2) && ((ram4.spk_conf2 & 0x06) == 0x06))) {
    ...
    /**************************/
        ...
        if (next_spark.tooth == tooth_no) {
            ...
            if (synch & (SYNC_WCOP | SYNC_WCOP2)) {
                if (num_spk == 4) {
                    ...
                    set_coil(&tmp_coil, &tmpcoilsel);
                } else if ((num_spk > 2) && (synch & SYNC_WCOP2)) { // 3,5,7 
                    tmpcoilsel = 0; // cancel any spark until out of WCOP mode
                }
            }
        }
        ...
        if ((next_dwell.tooth == tooth_no) && ((ram4.dwellmode & 3) != 2)) {
            thistoothevents |= DWELL;
            tmpdwellsel = 0;
            if (!(mapadc_thresh && (outpc.status1 & STATUS1_SYNCFULL) && ((next_dwell.coil + 1) > (ram4.no_cyl & 0x1f)))) {
            ...
            if (((synch & SYNC_WCOP) && (synch & SYNC_SEMI2)) || (synch & SYNC_WCOP2)) {
                if (num_spk == 4) {
                    ...
                    set_coil(&tmp_coil, &tmpdwellsel);
                } else if (num_spk > 2) { // 3,5,7,9,11,13,15
                    tmpdwellsel = 0; // cancel any dwell until out of WCOP mode
                }
            }
        }
    ...
    if (ram4.sequential & 0x3) {
        ...
        if (do_dualouts) {
            ...
            if (((num_cyl <= 4) || ((num_cyl > 4) && (ram4.sequential & SEQ_SEMI))) &&
...
//            time_mms = (next_dwell.time32 / 128);
    ...
    if (thistoothevents & ROTARY_DWL) {
        ...
        if (spkmode != 14) {
            if (next_dwl_trl.coil == (num_cyl<<1) - 1) {
                next = num_cyl;
            } else {
    ...
    if (thistoothevents & ROTARY_SPK) {
        ...
        if (spkmode != 14) {
            if (next_spk_trl.coil == (num_cyl << 1) - 1) {
                next = num_cyl;
            } else {
...
//            do_maplog_tooth(); // record tooth no. - this ends up being +1, so don't record here
        }
...
ALS_FUEL_CUT:;
  ...
  START_INJ:
    ...
    // Turn on fuel pump
    if (flagbyte6 & FLAGBYTE6_DONEINIT) {
        *port_fp |= pin_fp;
...
NO_TACHSET:
    ...
    *pPTMpin3 |= 0x08; /* Turn on Inj LED ... more like tacho LED now */
    ...
    if ((outpc.engine & ENGINE_CRANK) || (ram4.sequential & 0x3))
        goto SCHED_SQUIRT;
    ...
    if (divider == 1) {
        ...
        if (igncount < divider) {
            if (!(flagbyte2 & flagbyte2_EAElag)) {
                if ((EAElagcomp_squirts > 0)
                    && (EAElagcomp_squirts < num_cyl)) {
                    EAElagcomp_squirts++;
                    EAElag_squirting = 1;
...
SCHED1:
    ...
    if (EAElag_squirting) {
        ...
        inj1cntdown.time_32_bits = injtime_EAElagcomp;
    } else {
        outpc.pw1 = pwcalc1;
    }
    igncount = 0;
  ...
  SCHED2:
    ...
    if (EAElag_squirting) {
        ...
        inj2cntdown.time_32_bits = injtime_EAElagcomp;
    } else {
        outpc.pw2 = pwcalc2;
    }
