/* $Id: ms3_ign.c,v 1.192.2.4 2013/05/22 13:48:01 jsmcortina Exp $
...
void ign_reset(void)
{
    ...
    flagbyte17 &= ~(FLAGBYTE17_OVERRUNFC | FLAGBYTE17_REVLIMFC);
    knk_clk = 0;
    knk_clk_test = ram4.knk_trtd;
    knk_stat = 0;
    knk_count = 0;
    outpc.knk_rtd = 0;
    ...
    if (!(ram4.hardware & HARDWARE_MS3XFUEL)) {
       ...
       // using MS3X for fuel, turn them off
        ...
        if ((ram4.staged & 0x7) && (num_cyl > 4) && !(ram4.sequential & SEQ_SEMI)) {
            PORTT &= ~0x0A;
        }
        CSEM0;
    }
...
void ign_kill()
{
    ...
    *port_fp &= ~pin_fp;             // Turn off fuel Pump. // try again here
    outpc.engine = 0;
    outpc.rpm = 0;
                            ...
                            unsigned char rotary)
{
    ...
    for (i = start; i < stop; i++) {
        ...
        if (ram4.spk_mode3 & SPK_MODE3_SPKTRIM) {
            ...
            spk_ang = spk_ang_in + (int)trimtmp;
            ...
            if (spk_ang < MIN_IGN_ANGLE) {
                trimtmp = (char) ((int)trimtmp - spk_ang + MIN_IGN_ANGLE);
            ...
            if ((spkmode == 2) || (spkmode == 3) || (spkmode == 14)) { // dizzy or twin trigger
                if (ram4.adv_offset < 200) { // next-cyl
                    if (spk_ang < (ram4.adv_offset + 10)) {
                        trimtmp = (char) ((int)trimtmp + (ram4.adv_offset + 10) - spk_ang);
                        spk_ang = ram4.adv_offset + 10;
                    }
                } else { // this cyl
                    if (spk_ang > (ram4.adv_offset - 50)) {
                        trimtmp = (char) ((int)trimtmp + (ram4.adv_offset - 50) - spk_ang);
                        spk_ang = ram4.adv_offset - 50;
                    }
                }
            }
            dwl_ang = dwl_ang_in + (int)trimtmp;
        }
...
//    outpc.status4 = knk_clk; // DEBUG
    ...
    if (knockin > thresh) {
        ...
        if ((knk_stat == 0) && (knk_count > ram4.knk_ndet)) {
            knk_clk = knk_clk_test; // just caught knock - force 1st retard now
        }
    } else {
        outpc.status7 &= ~STATUS7_KNOCK;
    }
    ...
    if (knk_clk >= knk_clk_test) {  // time to check for retard/ adv spark
        if (knk_count > ram4.knk_ndet) {    // got valid knock. req ndet consec
            // knocks to be sure
            knk_clk_test = ram4.knk_trtd;
            ...
            knk_clk_test = ram4.knk_tadv;   // change time interval
            ...
            if ((knk_stat == 1) || (knk_stat == 2)) {
                ...
                if ((knk_stat == 2) && (outpc.knk_rtd == 0)) {
                    ...
                    if (outpc.knk_rtd >= ram4.knk_step2) {
                        outpc.knk_rtd -= ram4.knk_step2;    // remove some retard
                    } else {
                        outpc.knk_rtd = 0;
                    }
...
void calc_advance(long *lsum)
{
    ...
    if ((ram5.dualfuel_sw & 0x05) == 0x05) { // on + spark
        if (pin_tss && !(*port_tss & pin_tss)) {       // Hardware table switching
            uctmp = 1;
        }
    } else {
        if (ram4.feature5_0 & 0x10) {       // table switching
            if ((ram4.feature5_0 & 0x60) == 0) {
                if (pin_tss && !(*port_tss & pin_tss)) {       // Hardware table switching
                    uctmp = 1;
                }
            } else if ((ram4.feature5_0 & 0x60) == 0x20) {
                if (outpc.rpm > ram4.tss_rpm) {
                    uctmp = 1;
                }
            } else if ((ram4.feature5_0 & 0x60) == 0x40) {
                if (outpc.map > ram4.tss_kpa) {
                    uctmp = 1;
                }
            } else if ((ram4.feature5_0 & 0x60) == 0x60) {
    ...
    if (outpc.engine & ENGINE_CRANK) {
        ...
        *lsum = als_timing;
        ...
        if ((ram4.feature5_0 & 0x10) && ((ram4.feature5_0 & 0x60) == 0) && (ram4.tsw_pin_s & 0x1f) == 14) { /* blend from Spk1+2 to Spk3+4 */
            w = 3;
            ...
            if ((!(ram4.idleveadv_to_pid & IDLEVEADV_TO_PID_IDLEADV) && 
                ...
                if (((idle_advance_timer >= ram4.idleadvance_delay) &&
                    ...
                    if (ram4.idle_special_ops & IDLE_SPECIAL_OPS_CLIDLE_TIMING_ASSIST) {
                        *lsum += intrp_1ditable(outpc.rpm - outpc.cl_idle_targ_rpm, 
        ...
        *lsum -= intrp_1dctable(outpc.airtemp, NO_MAT_TEMPS, (int *) ram_window.pg8.MatTemps, 0, (unsigned char *) ram_window.pg8.MatSpkRtd, 8);    // %
        ...
        *lsum += ffspkdel;      // degx10
        ...
        if (ram4.RevLimOption & 1) {
            if (outpc.rpm > RevLimRpm2) {
                *lsum -= ram4.RevLimMaxRtd;     // deg x 10
                outpc.status3 |= STATUS3_REVLIMSFT;
            } else if (outpc.rpm > RevLimRpm1) {
                *lsum -= (((long) ram4.RevLimMaxRtd * (outpc.rpm - RevLimRpm1)) / (RevLimRpm2 - RevLimRpm1));   // deg x 10
                outpc.status3 |= STATUS3_REVLIMSFT;
            } else {
                outpc.status3 &= ~STATUS3_REVLIMSFT;
            }
...
void do_everytooth_calcs(long *lsum, long *lsum1, char *localflags)
{
    ...
    if (tooth_counter_main != tooth_counter) {
        ...
        longtmp2 = (longtmp2 * 100) / (long) ultmp;
        if (longtmp2 < -127) {
            longtmp2 = -127;
    ...
    if (synch & SYNC_SYNCED) {
        /* FIXME this code will be replaced when angle-clock implemented*/
        ...
        if (ram_window.pg24.vvt_opt1 & 0x03) {
            ...
            for (x = 0; x < n ; x++) {
                ...
                if (spkmode == 4) {
                    unsigned int opt = ram_window.pg24.vvt_tth[x];
                    if (opt && (opt < 25)) {
                        period = 7200 / opt;
                    } else {
                        period = 7200;
                ...
                if (((ang > (ram_window.pg24.vvt_max_ang[x] + 50)) || (ang < (ram_window.pg24.vvt_min_ang[x] - 50)))
                    ...
                    outpc.vvt_ang[x] = ang - ram_window.pg24.vvt_min_ang[x];
                }
            }
        }
        /* end FIXME */
...
//                        if (ram4.spk_conf2 & 0x10) { // note this bitfield has changed
                            rpm = Rpm_Coeff /
...
//                        }
            ...
            } else if ((outpc.engine & ENGINE_CRANK) || (outpc.rpm < 3)) {
    ...
    if ((last_tooth_time.time_32_bits != 0) && (dtpred_local)) {
        ...
        spk_time.time_32_bits = inttmp * ticks_per_deg; // needed on two lines or miscalculates
        spk_time.time_32_bits = spk_time.time_32_bits / 1000L;

        if ((ram4.dwellmode & 3) == 1) {
            maxdwl = 0; // no overdwell with fixed duty
...
//            if (outpc.engine & ENGINE_CRANK) { // preferred method
        ...
        if (((ram4.EngStroke & 0x03) == 0x03)) {
            ...
            dwl_time_tmp = (unsigned long) outpc.dwell_trl * 100;
            if (dwl_time_tmp > max_dwl_time) {
...
void calc_spk_trims(void)
{
    ...
    if ((fire < 1) || (fire > (ram4.no_cyl & 0x1f))) {
        fire = 1; //failsafe
    }
    fire--;
...
#ifdef KNOCK_DEBUG
            debug_str("KS: Invalid response, bailing.\r");
#endif
            stat_knock |= 1;
...
#ifdef KNOCK_DEBUG
            debug_str("KS5 > ");
            debug_bytehex(ram5.knock_bpass & 0x3f);
            debug_str("\r");
#endif
            knock_state++;
        }
...
#ifdef KNOCK_DEBUG
        debug_str("KS09 > ");
        debug_bytehex((ram5.knock_int & 0x1f) | 0xc0);
        debug_str("\r");
#endif
        knock_state++;
...
//        }
#ifdef KNOCK_DEBUG
        debug_str("knock_retry = ");
        debug_bytehex(knock_retry);
        debug_str("\r");
#endif
        knock_retry++;
        if (knock_retry > 3) {
