; crc32.s
;*********************************************************************
; $Id: crc32.s,v 1.10 2012/11/01 20:18:40 jsmcortina Exp $
...
;    for (i=0; i < bufLen; i++) {
    ; use 0x280c as i = buf, now use 0x2810 as down counter

;    tbeq	Y, .gLM7 ; 0 means full 64k

.gLM5:
;        crc32 = (crc32 >> 8) ^ crcTable[ (crc32 & 0xff) ^ byteBuf[i] ];
;    ldd     0x2809      ; crc32>>8
;    std     0x10, SP    ; (don't) save the 24bits into the 32bit temp space
    clra
    ldab    0x2808
...
;    clra ; already clear
    ...
    ldx     0x2809  ; do this now instead of load/store/load
    eorx    0, y
    stx	    0x280a
    std     0x2808 ; store that X from before
