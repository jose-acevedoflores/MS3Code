...
{
    int ix;
    if ((ram4.egt_num & 0x1f) == 0) {
        return;
    }
    for (ix = 0; ix < (ram4.egt_num & 0x1f) ; ix++) {
        port_egt[ix] = (unsigned short*)&dummyReg;
        generic_adc_setup(&port_egt[ix], 117, ram4.egtport[ix] & 0x1f, 69 + (ram4.egtport[ix] & 0x1f)); 
    }
}
