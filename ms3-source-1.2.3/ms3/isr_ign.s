; $Id: isr_ign.s,v 1.72 2012/11/26 16:32:46 jsmcortina Exp $
...
;Cam interrupts and some other low-level S12X ignition code.
  ...
  .sect .text3      ; was .text
  .globl ISR_Dwl_TimerOut, ISR_TC6, ISR_TC5, ISR_TC4, ISR_TC2, do_complog_pri
...
cmode15:
;Chrysler 2.2/2.5 'cam' detection (sensor 2)
;Do the same decoding as the 'crank' (sensor 1)

    ldab    synch
    andb	#SYNC_SYNCED
    bne     gol1

;check if this, last and last_1 tooth are all non-zero
    tstw    1,sp
    bne     cm15_1
    tstw    3,sp
    beq     gol1
cm15_1:
    tstw    cam_last_tooth
    bne     cm15_2
    tstw    cam_last_tooth+2
    beq     gol1
cm15_2:
    tstw    cam_last_tooth_1
    bne     cm15_nz
    tstw    cam_last_tooth_1+2
    beq     gol1
...
;                && (tooth_diff_this < (tooth_diff_last_1 >> 1)) && (tooth_diff_last > (tooth_diff_last_1 >> 1))) {
    ...
    bhi	gol1
    bcs	cm15_3
    cpd	cam_last_tooth+2
    bcc	gol1
cm15_3:
    ldd	cam_last_tooth+2
    ldx	cam_last_tooth
    cpx	cam_last_tooth_1
    bhi	gol1
    bcs	cm15_4
    cpd	cam_last_tooth_1+2
    bcc	gol1
cm15_4:
    ldd	cam_last_tooth_1
    lsrd
    ldx	cam_last_tooth_1+2
    rorx
    cpd	1,sp
    bcs	gol1
    bhi	cm15_5
...
cm15_5:
    ...
    bhi	gol1
    bcs	cm15_6
    cpx	cam_last_tooth+2
    bcc	gol1
cm15_6:
    movb    #4, tooth_no
    bset	outpc.status1, #STATUS1_SYNCFULL
    bset	synch, #SYNC_SYNCED
    movb    #2, outpc.status4 ; DEBUG
    bra     gol1
...
;GM7X cam detection
    ...
    tst     0,sp
    beq     cmode25_end    ; edge==0

;tooth_last cam_last_tooth
;tooth_this 1,sp

; Determine if pattern is long/short or short-long on the active edge
; if tooth_last > (tooth_this <<1) then long-short
; if tooth_this > (tooth_last <<1) then short-long
; else neither
...
;tt<<1
    ...
    cmpx    cam_last_tooth  ; check top word
    bhi     cm25b   ; test failed
    blo     cm25a   ; test passed

    cmpd    cam_last_tooth+2 ; low word
    bhi     cm25b   ; test failed
    beq     cm25b   ; test failed
...
;tl<<1
    ...
    cmpx    cam_last_tooth  ; check top word
    blo     cmode25_end   ; test failed
    bhi     cm25c   ; test passed

    cmpd    cam_last_tooth+2 ; low word
    blo     cmode25_end   ; test failed
    beq     cmode25_end   ; test failed
...
str_cam3:
    ...
    movb    tooth_no, cam_tooth+2 ; save the last tooth we saw before this cam input cam_tooth[1]
    subx    TC_crank+2
...
str_cam4:
    ...
    movb    tooth_no, cam_tooth+3 ; save the last tooth we saw before this cam input cam_tooth[1]
    subx    TC_crank+2
